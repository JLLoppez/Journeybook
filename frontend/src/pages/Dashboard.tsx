    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <div className="flex gap-2">
          <button onClick={() => navigate('/planner')} className="btn-outline text-sm py-2.5">
            ✨ AI Planner
          </button>
          <button onClick={() => navigate('/flights')} className="btn-primary text-sm py-2.5">
            + Book Flight
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          { label: 'Total Trips', value: active.length, icon: '✈', color: 'text-sky-600', bg: 'bg-sky-50' },
          { label: 'Upcoming', value: upcoming.length, icon: '📅', color: 'text-emerald-600', bg: 'bg-emerald-50' },
          { label: 'Past Trips', value: past.length, icon: '🗺️', color: 'text-violet-600', bg: 'bg-violet-50' },
          { label: 'Total Spent', value: `R${totalSpent.toLocaleString()}`, icon: '💳', color: 'text-amber-600', bg: 'bg-amber-50' },
        ].map((s) => (
          <div key={s.label} className="card p-5">
            <div className={`w-9 h-9 rounded-xl ${s.bg} flex items-center justify-center text-lg mb-3`}>
              {s.icon}
            </div>
      ...
      {nextTrip?.flight && (
        ...
        >
          ...
          <div className="absolute -right-6 -top-6 text-[120px] opacity-[0.04] pointer-events-none select-none">✈</div>
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="text-white/60 text-sm mt-1">
                  {nextTrip.flight.destination?.city} ·{' '}
                  {nextTrip.flight.departure?.date
              ...
              <span className="px-4 py-2 rounded-xl text-sm font-bold text-white transition-colors"
        ...
        <div className="card p-16 text-center">
          ...
          {tab !== 'past' && (
            <div className="flex gap-3 justify-center">
              <button onClick={() => navigate('/planner')} className="btn-outline text-sm">✨ AI Planner</button>
              <button onClick={() => navigate('/flights')} className="btn-primary text-sm">Search Flights</button>
            </div>
          )}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 fade-in">
          {shown.map((b) => (
            <BookingCard
              key={b._id}
              booking={b}
              onCancel={b.status === 'confirmed' ? handleCancel : undefined}
            />
          ))}
        </div>
      )}
