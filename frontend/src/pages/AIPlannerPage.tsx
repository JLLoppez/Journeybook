...
import {
  Plane, MapPin, Users, Calendar, ArrowRight, ArrowLeftRight,
  Briefcase, Lightbulb, FileText, Hotel, Sparkles, Bot,
  Clock, Wallet, User, RotateCcw, CheckCircle, ChevronRight,
  AlertCircle
} from 'lucide-react';
...
const TRIP_TYPES = [
  { value: 'solo',     emoji: '🧍', name: 'Solo' },
  { value: 'couple',   emoji: '👫', name: 'Couple' },
  { value: 'family',   emoji: '👨‍👩‍👧', name: 'Family' },
  { value: 'friends',  emoji: '👯', name: 'Friends' },
...
export default function AIPlannerPage() {
  ...
  const validate = (): boolean => {
    ...
    if (isReturn && !returnDate) { toast.error('Please select a return date'); return false; }
  ...
  const handleSelectPlan = () => {
    ...
    if (isReturn && (plan.returnDateSuggested || returnDate)) {
      params.set('returnDate', plan.returnDateSuggested || returnDate);
    }
  ...
  const statItems = plan ? [
    { icon: <Wallet className="w-4 h-4" />, label: 'Total Budget', zar: formatZAR(plan.totalBudgetZAR || totalCostZAR), converted: isZAR ? null : formatAmount(plan.totalBudgetZAR || totalCostZAR) },
    { icon: <User className="w-4 h-4" />, label: 'Per Person', zar: formatZAR(plan.perPersonBudgetZAR || Math.round(totalCostZAR / Math.max(totalPax, 1))), converted: isZAR ? null : formatAmount(plan.perPersonBudgetZAR || Math.round(totalCostZAR / Math.max(totalPax, 1))) },
    { icon: <Calendar className="w-4 h-4" />, label: 'Departure', zar: plan.departureDateSuggested || departureDate, converted: null },
    { icon: isReturn ? <RotateCcw className="w-4 h-4" /> : <Plane className="w-4 h-4" />, label: isReturn ? 'Return' : 'One Way', zar: isReturn ? (plan.returnDateSuggested || returnDate || '—') : 'No return', converted: null },
  ] : [];
      ...
      <div className="card p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          <div>
            ...
            <CitySearch value={originDisplay} onChange={(d, code) => { setOriginDisplay(d); setOriginCode(code); }} placeholder="Cape Town, Johannesburg..." />
          </div>
          <div>
            <label className="label">Destination *</label>
            <CitySearch value={destDisplay} onChange={(d, code) => { setDestDisplay(d); setDestCode(code); }} placeholder="Dubai, London, Zanzibar..." />
          </div>
        </div>

        {/* Row 2: Trip type + One way/Return */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {TRIP_TYPES.map(t => (
                <button key={t.value} type="button"
                  ...
                  className={'flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 text-sm font-semibold transition-all ' + (tripType === t.value ? 'border-sky-500 bg-sky-50 text-sky-700' : 'border-slate-200 text-slate-600 hover:border-sky-200')}
                >
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setIsReturn(false)}
                className={'flex-1 py-2.5 rounded-xl border-2 text-sm font-bold transition-all flex items-center justify-center gap-2 ' + (!isReturn ? 'border-sky-500 bg-sky-50 text-sky-700' : 'border-slate-200 text-slate-500 hover:border-sky-200')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          <div>
            <label className="label flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> Passengers</label>
            <PassengerSelector value={passengers} onChange={setPassengers} />
          </div>
          <div>
            <label className="label flex items-center gap-1.5"><Briefcase className="w-3.5 h-3.5" /> Cabin Class</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
          <div>
            <label className="label flex items-center gap-1.5">
              ...
              <span className="text-slate-400 font-normal text-xs ml-1">(change in navbar)</span>
            </label>
        ...
        <button onClick={handleGenerate} disabled={loading}
          className="w-full py-4 text-white font-bold rounded-xl text-base transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl hover:-translate-y-0.5 flex items-center justify-center gap-2"
          style={{ background: 'linear-gradient(135deg, #0ea5e9, #6366f1)', fontFamily: 'Syne, sans-serif' }}>
          {loading ? (
            <><div className="w-5 h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Planning your trip...</>
          ) : (
            <><Sparkles className="w-5 h-5" /> Generate My {isReturn ? 'Return' : 'One-Way'} Trip Plan</>
          )}
        </button>
      </div>
        ...
        <div className="card p-10 text-center mb-8">
          ...
          <div className="w-full max-w-xs mx-auto bg-slate-100 rounded-full h-2 overflow-hidden">
            <div className="h-2 rounded-full transition-all duration-700"
              style={{ width: ((loadingStep + 1) / LOADING_STEPS.length * 100) + '%', background: 'linear-gradient(90deg, #0ea5e9, #6366f1)' }} />
          </div>
        ...
        <div id="plan-results" className="space-y-5 fade-up">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-2 text-white/70 text-sm">
                      <span className="flex items-center gap-1">
                        {isReturn ? <ArrowLeftRight className="w-3.5 h-3.5" /> : <ArrowRight className="w-3.5 h-3.5" />}
                        {isReturn ? 'Return' : 'One Way'}
                      </span>
                      <span>·</span>
                      <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {plan.duration}</span>
                      <span>·</span>
                      <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" /> {totalPax} passenger{totalPax !== 1 ? 's' : ''}</span>
                      <span>·</span>
                      <span className="flex items-center gap-1 capitalize"><Briefcase className="w-3.5 h-3.5" /> {cabinClass}</span>
                    </div>
                  </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2" style={{ fontFamily: 'Syne, sans-serif' }}>
                <MapPin className="w-5 h-5 text-sky-500" /> Day-by-Day Itinerary
              </h3>
              {totalCostZAR > 0 && (
                  ...
                  <div className="flex-1 pb-5 border-b border-slate-100 last:border-0 last:pb-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap mb-2">
                      ...
                      {day.estimatedCostZAR && (
                        <div className="text-right flex-shrink-0">
                          ...
                          {!isZAR && <div className="text-xs text-slate-400 mt-0.5 text-right">{formatAmount(day.estimatedCostZAR)}</div>}
                        </div>
                      )}
                    </div>
                    <ul className="space-y-1.5">
                      {day.activities.map((a, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-slate-600">
                          <ChevronRight className="w-3.5 h-3.5 text-sky-400 mt-0.5 flex-shrink-0" />
                          <span>{a}</span>
                        </li>
                      ))}
                    </ul>
                    {day.accommodation && (
                      <div className="mt-2 flex items-center gap-1.5 text-xs text-slate-400 font-medium">
                        <Hotel className="w-3.5 h-3.5" /> {day.accommodation}
                      </div>
                    )}
                  </div>
                </div>
            ...
            {totalCostZAR > 0 && (
              <div className="mt-5 pt-4 border-t border-slate-100 space-y-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-extrabold text-slate-900" style={{ fontFamily: 'Syne, sans-serif' }}>{formatZAR(totalCostZAR)}</span>
                </div>
                {!isZAR && (
                  <div className="flex justify-between text-sm">
          ...
          {(plan.flights?.length ?? 0) > 0 && (
            <div className="card p-6">
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2" style={{ fontFamily: 'Syne, sans-serif' }}>
                <Plane className="w-5 h-5 text-sky-500" /> Matching Flights in Our System
              </h3>
                ...
                {plan.flights.map((f: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{f.origin?.code} <ArrowRight className="w-3 h-3 inline" /> {f.destination?.code} · {f.duration} · {f.class}</p>
                    </div>
                    <div className="text-right">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {(plan.tips?.length ?? 0) > 0 && (
              <div className="card p-5">
                <h3 className="text-base font-bold text-slate-900 mb-4 flex items-center gap-2" style={{ fontFamily: 'Syne, sans-serif' }}>
                  <Lightbulb className="w-4 h-4 text-amber-400" /> Travel Tips
                </h3>
                <ul className="space-y-2.5">
                  {plan.tips?.map((t, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-sm text-slate-600">
                      <CheckCircle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0 mt-0.5" /> {t}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {(plan.requiredDocuments?.length ?? 0) > 0 && (
              <div className="card p-5">
                <h3 className="text-base font-bold text-slate-900 mb-4 flex items-center gap-2" style={{ fontFamily: 'Syne, sans-serif' }}>
                  <FileText className="w-4 h-4 text-sky-500" /> Documents Required
                </h3>
                <ul className="space-y-2.5">
                  {plan.requiredDocuments?.map((d, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-sm text-slate-600">
                      <AlertCircle className="w-3.5 h-3.5 text-sky-400 flex-shrink-0 mt-0.5" /> {d}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="relative flex flex-col sm:flex-row items-center justify-between gap-5">
              <div>
                ...
                <p className="text-slate-500 text-sm leading-relaxed">
                  Click below to go to flight search with all your details pre-filled.
                </p>
                <div className="flex flex-wrap gap-3 mt-3 text-xs text-slate-500">
                  <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> {originDisplay || originCode} <ArrowRight className="w-3 h-3" /> {destDisplay || destCode}</span>
                  <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> {departureDate}{isReturn ? ' → ' + returnDate : ' (one way)'}</span>
                  <span className="flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> {totalPax} pax · {cabinClass}</span>
                </div>
              </div>
              <div className="flex flex-col gap-3 flex-shrink-0 w-full sm:w-auto">
                <button onClick={handleSelectPlan}
                  className="px-8 py-4 text-white font-extrabold rounded-2xl text-base transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5 flex items-center justify-center gap-2"
                  style={{ background: 'linear-gradient(135deg, #0ea5e9, #6366f1)', fontFamily: 'Syne, sans-serif', minWidth: '220px' }}>
                  <Plane className="w-5 h-5" /> Select This Plan
                </button>
                <button onClick={() => { setPlan(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
