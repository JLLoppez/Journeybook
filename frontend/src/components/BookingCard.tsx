...
export default function BookingCard({ booking, onCancel }: BookingCardProps) {
  ...
  return (
    <div className="card-hover group" onClick={() => navigate('/bookings/' + booking._id)}>
      <div className={'h-1 ' + (booking.class === 'first' ? 'bg-gradient-to-r from-violet-500 to-purple-600' : booking.class === 'business' ? 'bg-gradient-to-r from-amber-400 to-orange-500' : 'bg-gradient-to-r from-sky-500 to-blue-600')} />

      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-slate-400 mb-0.5 font-medium">Booking ref</p>
            <p className="font-bold text-slate-900 tracking-wider text-sm" style={{ fontFamily: 'Syne, sans-serif', letterSpacing: '0.06em' }}>
        ...
        {flight ? (
          <>
            <div className="flex items-center gap-3 mb-4">
              <div>
                <p className="text-2xl font-black text-slate-900" style={{ fontFamily: 'Syne, sans-serif', letterSpacing: '-0.02em' }}>
                  {flight.origin?.code}
                </p>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              <div className="flex items-center gap-3 text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  ...
                  {safeFormat(flight.departure?.date, 'dd MMM yyyy')}
                </span>
                <span className="flex items-center gap-1">
                  <Users className="w-3.5 h-3.5" />
                  {booking.passengers}x
                </span>
              </div>
              <p className="font-extrabold text-sky-600" style={{ fontFamily: 'Syne, sans-serif' }}>
